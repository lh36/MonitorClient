...
public class DrawManager : SingletonUnity<DrawManager>
{
   ...
   // public LineRenderer[] m_ShipLineRenderer = new LineRenderer[3];//船轨迹线
	public LineRenderer m_ShipLineRenderer1;
	public LineRenderer m_ShipLineRenderer2;
	public LineRenderer m_ShipLineRenderer3;
    ...
    private Dictionary<int, List<Vector3>> m_ShipTrackDict;//船航迹路径点
    ...
    private void TrackDraw(object oSender, object oParam)
    {
       ...
       // var iLength1 = this.m_ShipTrackDict[ShipManager.Instance.ControlShipID].Count;//计算长度
		var iLength1 = this.m_ShipTrackDict[1].Count;
		var iLength2 = this.m_ShipTrackDict[2].Count;
		var iLength3 = this.m_ShipTrackDict[3].Count;
        this.m_ShipLineRenderer1.SetVertexCount (iLength1);//设置长度
		this.m_ShipLineRenderer2.SetVertexCount (iLength2);
		this.m_ShipLineRenderer3.SetVertexCount (iLength3);
        for(int i=0; i < iLength1; i++)
        {
            this.m_ShipLineRenderer1.SetPosition (i, this.m_ShipTrackDict[1][i]);
        }
		for(int i=0; i < iLength2; i++)
		{
			this.m_ShipLineRenderer2.SetPosition (i, this.m_ShipTrackDict[2][i]);
		}
		for(int i=0; i < iLength3; i++)
		{
			this.m_ShipLineRenderer3.SetPosition (i, this.m_ShipTrackDict[3][i]);
		}
        //TrackDrawByID(this.m_ShipLineRenderer, this.m_ShipTrackDict);
    }

    public void TrackDrawByID(LineRenderer[] render, Dictionary<int, List<Vector3>> m_ShipTrackDict)
    {

        var iLength1 = m_ShipTrackDict[1].Count;//计算长度
        var iLength2 = m_ShipTrackDict[2].Count;//计算长度
        var iLength3 = m_ShipTrackDict[3].Count;//计算长度
        render[0].SetVertexCount(iLength1);//设置长度
        for (int j = 0; j < iLength1; j++)
        {
            render[0].SetPosition(j, m_ShipTrackDict[1][j]);
        }

        render[1].SetVertexCount(iLength2);//设置长度
        for (int j = 0; j < iLength2; j++)
        {
            render[0].SetPosition(j, m_ShipTrackDict[2][j]);
        }

        render[2].SetVertexCount(iLength3);//设置长度
        for (int j = 0; j < iLength3; j++)
        {
            render[0].SetPosition(j, m_ShipTrackDict[3][j]);
        }
    }
